...
最小合法命令（原样使用，禁止改参数名）：
```bash
codex exec -C "$PROJECT_DIR" -s read-only --skip-git-repo-check -o "$OUTPUT_FILE" "[你的 prompt]"
```
完整模板见 [`references/cli-examples.md`](./references/cli-examples.md)
...
7. 命令生成规范：必须使用 `codex exec` 子命令；禁止使用 `-q` 或其他不存在的 flag；不确定时原样复制模板，禁止自行重构
